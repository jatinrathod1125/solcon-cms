...
@section('header-title', 'Modify Machine')

@section('content')
<div class="max-w-2xl mx-auto space-y-6">
    <!-- Back Link -->
    <div>
        <a href="{{ route('admin.machines.index') }}" class="inline-flex items-center text-sm text-slate-400 hover:text-white transition-colors gap-2">
    ...
    <div class="bg-slate-950 border border-slate-850 rounded-2xl shadow-xl p-6 md:p-8">
        <h3 class="text-lg font-bold text-white mb-6">Modify Machine Details</h3>

        <!-- Form -->
        <form method="POST" action="{{ route('admin.machines.update', $machine) }}" class="space-y-6">
            @csrf
            @method('PUT')
