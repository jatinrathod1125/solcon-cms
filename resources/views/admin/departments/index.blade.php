...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <!-- Search Form -->
        <form method="GET" action="{{ route('admin.departments.index') }}" class="flex items-center w-full sm:max-w-md gap-2">
            ...
            @if(request('search'))
                <a href="{{ route('admin.departments.index') }}" class="px-3 py-2 bg-slate-900 text-slate-400 hover:text-white rounded-xl text-sm transition-colors" title="Clear Search">
                    Clear
                </a>
            @endif
        </form>
                        ...
                        <tr class="hover:bg-slate-900/30 text-slate-200 transition-colors">
                            ...
                            <td class="p-4 text-right">
                                <div class="flex items-center justify-end space-x-2">
                                    <a href="{{ route('admin.departments.edit', $department) }}" class="p-1.5 bg-slate-900 border border-slate-800 hover:border-cyan-500/40 text-slate-400 hover:text-cyan-400 rounded-lg transition-all" title="Edit">
                                        <i data-lucide="edit-2" class="w-4 h-4"></i>
                                    </a>
                                    <form method="POST" action="{{ route('admin.departments.destroy', $department) }}" class="delete-form inline-block">
                                        @csrf
                                        @method('DELETE')
...
<script>
    $(document).ready(function() {
        $('.delete-btn').on('click', function(e) {
            ...
            Swal.fire({
                title: 'Are you sure?',
                text: "Deleting this department cannot be undone.",
                icon: 'warning',
