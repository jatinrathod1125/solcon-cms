...
@section('title', 'Edit Component Formula')
...
<div class="max-w-4xl mx-auto space-y-6">
    <div class="flex items-center gap-3">
        ...
        <div>
            <h2 class="text-xl font-bold text-white">Edit Component Formula</h2>
            <p class="text-xs text-slate-500">Update the recipe ingredients and version configuration.</p>
        </div>
    </div>

    <div class="bg-slate-955 border border-slate-850 p-6 rounded-2xl shadow-xl">
        <form method="POST" action="{{ route('admin.epoxy-component-formulas.update', $epoxyComponentFormula->id) }}" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <select name="epoxy_component_id" required
                        ...
                        @foreach($components as $comp)
                            <option value="{{ $comp->id }}" {{ old('epoxy_component_id', $epoxyComponentFormula->epoxy_component_id) == $comp->id ? 'selected' : '' }}>
                                {{ $comp->name }} ({{ $comp->code }})
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="number" name="version" value="{{ old('version', $epoxyComponentFormula->version) }}" min="1" required
                ...
                <div class="flex items-center pt-6">
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $epoxyComponentFormula->is_active) ? 'checked' : '' }} class="sr-only peer">
            ...
            <div>
                ...
                <textarea name="description" rows="2" placeholder="Describe this formula variation..."
                    class="block w-full px-4 py-2.5 bg-slate-900 border border-slate-800 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-cyan-500/50 focus:border-cyan-500 text-sm">{{ old('description', $epoxyComponentFormula->description) }}</textarea>
            </div>
...
<script>
    $(document).ready(function() {
        var itemIndex = {{ count($epoxyComponentFormula->items) }};
