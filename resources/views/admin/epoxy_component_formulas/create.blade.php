    ...
    <div class="max-w-5xl mx-auto space-y-6">
        <div class="flex items-center gap-3">
            <a href="{{ route('admin.epoxy-component-formulas.index') }}"
                class="p-2 hover:bg-slate-900 rounded-xl text-slate-400 hover:text-white transition-all">
                <i data-lucide="arrow-left" class="w-5 h-5"></i>
            </a>
            <div>
                <h2 class="text-xl font-bold text-white">Define Component Formula</h2>
                <p class="text-xs text-slate-500">Map the raw material and packaging ingredients to produce 1 unit of this
                    component.</p>
            </div>
        </div>
        ...
        <div class="bg-slate-955 border border-slate-850 p-6 rounded-2xl shadow-xl">
            <form method="POST" action="{{ route('admin.epoxy-component-formulas.store') }}" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Target
                            Component</label>
                        <select name="epoxy_component_id" required
                            ...
                            @foreach ($components as $comp)
                                <option value="{{ $comp->id }}"
                                    {{ old('epoxy_component_id', $preselectedComponentId) == $comp->id ? 'selected' : '' }}>
                                    {{ $comp->name }} ({{ $comp->code }})
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Version
                            Number</label>
                ...
                <div>
                    <label class="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Version Remarks /
                        Description</label>
                ...
                <div class="space-y-4 pt-4 border-t border-slate-850">
                    <div class="flex items-center justify-between">
                        <h3 class="text-sm font-bold text-white uppercase tracking-wider">Formula Ingredients (per 1 unit of
                            component)</h3>
                        <button type="button" id="add-item-btn"
                            class="inline-flex items-center px-3 py-1.5 bg-slate-900 hover:bg-slate-850 text-xs font-bold rounded-lg text-cyan-400 transition-colors border border-slate-800 gap-1.5">
        ...
        <tr class="item-row">
            ...
            <td class="py-3 pr-2">
                <select name="items[INDEX][item_type]"
                    class="item-type-select block w-full px-2.5 py-2 bg-slate-900 border border-slate-800 rounded-lg text-white text-xs focus:outline-none focus:ring-1 focus:ring-cyan-500">
            ...
            <td class="py-3 pr-2">
                ...
                <div class="raw-select-wrap">
                    <select name="items[INDEX][raw_material_id]"
                        class="block w-full px-3 py-2 bg-slate-900 border border-slate-800 rounded-lg text-white text-xs focus:outline-none focus:ring-1 focus:ring-cyan-500">
                ...
                <div class="packing-select-wrap hidden">
                    <select name="items[INDEX][packing_material_id]"
                        class="block w-full px-3 py-2 bg-slate-900 border border-slate-800 rounded-lg text-white text-xs focus:outline-none focus:ring-1 focus:ring-purple-500">
                        <option value="">-- Select Packing Material --</option>
                        @foreach ($packingMaterials as $pm)
                            <option value="{{ $pm->id }}">{{ $pm->name }}
                                ({{ $pm->category->name ?? 'Packing' }})</option>
                        @endforeach
                    </select>
                </div>
            </td>
            ...
            <td class="py-3 pr-2">
                <select name="items[INDEX][unit_id]" required
                    ...
                    @foreach ($units as $unit)
                        <option value="{{ $unit->id }}" {{ $unit->code === 'KG' ? 'selected' : '' }}>
            ...
            <td class="py-3 text-right">
                <button type="button"
                    class="remove-item-btn p-1.5 hover:bg-rose-500/10 text-slate-500 hover:text-rose-400 rounded-lg transition-colors">
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('change', '.item-type-select', function() {
                ...
                var type = $(this).val();
                if (type === 'raw') {
                    $row.find('.raw-select-wrap').removeClass('hidden').find('select').prop('required',
                        true);
                    $row.find('.packing-select-wrap').addClass('hidden').find('select').prop('required',
                        false).val('');
                } else {
                    $row.find('.packing-select-wrap').removeClass('hidden').find('select').prop('required',
                        true);
                    $row.find('.raw-select-wrap').addClass('hidden').find('select').prop('required', false)
                        .val('');
                }
            });
