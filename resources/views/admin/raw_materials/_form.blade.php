...
<!-- Section 2: Brand, Department and Units -->
<div class="grid grid-cols-1 md:grid-cols-3 gap-6">
    <!-- Brand -->
    <div>
        ...
        <select id="brand_id" name="brand_id"
            ...
            <option value="" {{ old('brand_id', isset($rawMaterial) ? $rawMaterial->brand_id : null) === null ? 'selected' : '' }}></option>
            @foreach($brands as $brand)
                <option value="{{ $brand->id }}" {{ old('brand_id', isset($rawMaterial) ? $rawMaterial->brand_id : null) == $brand->id ? 'selected' : '' }}>
                    {{ $brand->name }}
                </option>
            @endforeach
        </select>
