...
@section('title', 'Define Epoxy Formula')
@section('header-title', 'Create Epoxy Formula')
...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-6">
        <div>
            <h2 class="text-lg font-extrabold text-slate-900">Define Epoxy Formula</h2>
            <p class="text-xs text-slate-500">Map ingredients, quantities, and dynamic color flags for Epoxy assembly lines.</p>
        </div>

        <form action="{{ route('admin.epoxy-formulas.store') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div class="flex items-center justify-end gap-2 pt-4 border-t border-slate-100">
                ...
                <button type="submit" class="erp-button bg-blue-600 text-white hover:bg-blue-500">
                    <i data-lucide="plus" class="w-4 h-4"></i>Save Epoxy Formula
                </button>
            </div>
        </form>
    </div>
