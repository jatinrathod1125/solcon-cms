...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <form method="GET" action="{{ route('admin.grades.index') }}" class="flex items-center w-full sm:max-w-xl gap-2">
            <!-- Search Text -->
            ...
            @if(request('search') || request('status'))
                <a href="{{ route('admin.grades.index') }}" class="px-3 py-2 bg-slate-900 text-slate-400 hover:text-white rounded-xl text-sm transition-colors" title="Clear Search">
                    Clear
                </a>
            @endif
        </form>
                ...
                <tbody class="divide-y divide-slate-850/50">
                    @forelse($grades as $grade)
                        <tr class="hover:bg-slate-900/30 text-slate-200 transition-colors">
                            ...
                            <!-- Name & Description -->
                            <td class="p-4">
                                <div class="font-semibold text-white">{{ $grade->name }}</div>
                                <div class="text-xs text-slate-500 max-w-xs truncate">{{ $grade->description ?? '-' }}</div>
                            </td>
                            ...
                            <td class="p-4 text-xs text-slate-400">
                                <div>By: <span class="text-white font-medium">{{ $grade->creator->name ?? 'System' }}</span></div>
                            ...
                            <td class="p-4 text-right">
                                <div class="flex items-center justify-end space-x-2">
                                    <a href="{{ route('admin.grades.edit', $grade) }}" class="p-1.5 bg-slate-900 border border-slate-800 hover:border-cyan-500/40 text-slate-400 hover:text-cyan-400 rounded-lg transition-all" title="Edit">
                                        <i data-lucide="edit-2" class="w-4 h-4"></i>
                                    </a>
                                    <form method="POST" action="{{ route('admin.grades.destroy', $grade) }}" class="delete-form inline-block">
                                        @csrf
                                        @method('DELETE')
