<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
    <!-- Name -->
    <div>
        <label for="name" class="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Grade Name</label>
    ...
    <!-- Code -->
    <div>
        <label for="code" class="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Grade Code</label>
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6">
    ...
    <div>
        ...
        <select id="department_id" name="department_id" required
            ...
            @foreach($departments as $dept)
                <option value="{{ $dept->id }}" {{ old('department_id', $grade->department_id ?? '') == $dept->id ? 'selected' : '' }}>
                    {{ $dept->name }} ({{ $dept->code }})
                </option>
            @endforeach
        </select>
    ...
    <!-- Bag Size Mapping -->
    <div>
        <label for="bag_size_id" class="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Bag Size</label>
        <select id="bag_size_id" name="bag_size_id" required
            class="block w-full px-4 py-2.5 bg-slate-900 border border-slate-800 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-cyan-500/50 focus:border-cyan-500 transition-all text-sm">
            <option value="">Select Bag Size</option>
            @foreach($bagSizes as $bagSize)
                <option value="{{ $bagSize->id }}" {{ old('bag_size_id', $grade->bag_size_id ?? '') == $bagSize->id ? 'selected' : '' }}>
    ...
    <!-- Output Unit Mapping -->
    <div>
        <label for="output_unit_id" class="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Output Unit</label>
        <select id="output_unit_id" name="output_unit_id" required
            class="block w-full px-4 py-2.5 bg-slate-900 border border-slate-800 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-cyan-500/50 focus:border-cyan-500 transition-all text-sm">
            <option value="">Select Output Unit</option>
            @foreach($units as $unit)
                <option value="{{ $unit->id }}" {{ old('output_unit_id', $grade->output_unit_id ?? '') == $unit->id ? 'selected' : '' }}>
                    {{ $unit->name }} ({{ $unit->code }})
                </option>
            @endforeach
        </select>
