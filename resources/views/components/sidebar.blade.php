@php
    ...
    $canAccessAdhesive =
        !$user->isMarketing() &&
        !$user->isDispatch() &&
        ($user->isAdmin() || ($deptTAD && $user->canAccessDepartment($deptTAD->id)));
    $canAccessGrout =
        !$user->isMarketing() &&
        !$user->isDispatch() &&
        ($user->isAdmin() || ($deptGRT && $user->canAccessDepartment($deptGRT->id)));
    $canAccessEpoxy =
        !$user->isMarketing() &&
        !$user->isDispatch() &&
...
<aside id="appSidebar"
    ...
    <nav class="sidebar-scroll flex-1 space-y-6 overflow-y-auto px-3 py-5">
                ...
                <div class="space-y-1">
                    ...
                    @if ($user->isAdmin() || $user->isMarketing())
                        <a href="{{ route('dispatch.create') }}"
                            class="{{ $navClass(request()->routeIs('dispatch.create')) }}" title="Create Dispatch">
                            <i data-lucide="plus-circle"></i>
                            <span class="sidebar-label">Create Dispatch</span>
                        </a>
                    @endif
                    <a href="{{ route('dispatch.reports') }}"
                        class="{{ $navClass(request()->routeIs('dispatch.reports')) }}" title="Dispatch Reports">
            ...
            <section>
                ...
                <div class="space-y-1">
                    <a href="{{ route('production.planning') }}"
                        class="{{ $navClass(request()->routeIs('production.planning')) }}" title="Production Planning">
                    ...
                    <a href="{{ route('production.history') }}"
                        class="{{ $navClass(request()->routeIs('production.history')) }}" title="Production history">
        ...
        @if ($canAccessEpoxy)
            <section>
                ...
                <div class="space-y-1">
                    <a href="{{ route('epoxy.index') }}" class="{{ $navClass(request()->routeIs('epoxy.index')) }}"
                        title="Epoxy Floor">
                        <i data-lucide="activity"></i>
                        <span class="sidebar-label">Epoxy Floor</span>
                    </a>
                    <a href="{{ route('epoxy.component-entry') }}"
                        class="{{ $navClass(request()->routeIs('epoxy.component-entry')) }}" title="Component Entry">
                        <i data-lucide="plus-circle"></i>
                        <span class="sidebar-label">Component Entry</span>
                    </a>
                    <a href="{{ route('epoxy.bucket-assembly') }}"
                        class="{{ $navClass(request()->routeIs('epoxy.bucket-assembly')) }}" title="Bucket Assembly">
                ...
                <div class="space-y-1">
                    @if ($user->isAdmin())
                        <a href="{{ route('admin.raw-materials.index') }}"
                            class="{{ $navClass(request()->routeIs('admin.raw-materials.*')) }}"
                            title="Raw Materials">
                            <i data-lucide="box"></i>
                            <span class="sidebar-label">Raw Materials</span>
                        </a>
                        <a href="{{ route('admin.packing-materials.index') }}"
                            class="{{ $navClass(request()->routeIs('admin.packing-materials.*')) }}"
                    ...
                    <a href="{{ route('admin.stock-adjustments.index') }}"
                        class="{{ $navClass(request()->routeIs('admin.stock-adjustments.*')) }}"
                        title="Stock IN / OUT">
        ...
        @if ($user->isAdmin())
            <section>
                ...
                <div class="space-y-1">
                    ...
                    <details class="sidebar-more"
                        ...
                        <summary class="sidebar-link cursor-pointer">
                            ...
                            <i data-lucide="chevron-right"
                                class="sidebar-label ml-auto h-4 w-4 transition-transform"></i>
                        </summary>
                    ...
                    <details class="sidebar-more"
                        ...
                        request()->routeIs('admin.formulas.*') ||
                        request()->routeIs('admin.bag-sizes.*')
                            ? 'open'
                            : '' }}>
                        <summary class="sidebar-link cursor-pointer">
                        ...
                        <div class="sidebar-label ml-9 mt-1 space-y-1 border-l border-white/10 pl-3">
                            ...
                            <a href="{{ route('admin.bag-sizes.index') }}"
                                class="sub-link {{ request()->routeIs('admin.bag-sizes.*') ? 'is-active' : '' }}">Bag
                                Sizes</a>
                        </div>
                    </details>
                    ...
                    {{-- Grout Setup --}}
                    <details class="sidebar-more"
                        {{ request()->routeIs('admin.grout-colors.*') || request()->routeIs('admin.gro   rmulas.*') ? 'open' : '' }}>
                        <summary class="sidebar-link cursor-pointer">
                        ...
                        <div class="sidebar-label ml-9 mt-1 space-y-1 border-l border-white/10 pl-3">
                            <a href="{{ route('admin.grout-colors.index') }}"
                                class="sub-link {{ request()->routeIs('admin.grout-colors.*') ? 'is-active' : '' }}">Grout
                                Colors</a>
                            <a href="{{ route('admin.grout-formulas.index') }}"
                                class="sub-link {{ request()->routeIs('admin.grout-formulas.*') ? 'is-active' : '' }}">Grout
                                Formulas</a>
                        </div>
                    </details>
                    ...
                    <details class="sidebar-more"
                        ...
                        <div class="sidebar-label ml-9 mt-1 space-y-1 border-l border-white/10 pl-3">
                            <a href="{{ route('admin.epoxy-products.index') }}"
                                class="sub-link {{ request()->routeIs('admin.epoxy-products.*') ? 'is-active' : '' }}">Epoxy
                                Products</a>
                            <a href="{{ route('admin.epoxy-formulas.index') }}"
                                class="sub-link {{ request()->routeIs('admin.epoxy-formulas.*') ? 'is-active' : '' }}">Epoxy
                                Formulas</a>
                            ...
                            <a href="{{ route('admin.epoxy-component-formulas.index') }}"
                                class="sub-link {{ request()->routeIs('admin.epoxy-component-formulas.*') ? 'is-active' : '' }}">Component
                                Formulas</a>
                        </div>
                    </details>
                </div>
            ...
            {{-- Brand Management --}}
            <section>
                <p class="sidebar-label nav-eyebrow">Brand Management</p>
                <div class="space-y-1">
                    <a href="{{ route('brands.index') }}" class="{{ $navClass(request()->routeIs('brands.*')) }}"
                        title="Brands"><i data-lucide="tag"></i><span class="sidebar-label">Brands</span></a>
                </div>
            </section>
            ...
            <section>
                ...
                <div class="space-y-1">
                    <a href="{{ route('admin.users.index') }}"
                        class="{{ $navClass(request()->routeIs('admin.users.*')) }}" title="User management"><i
                            data-lucide="users"></i><span class="sidebar-label">Users</span></a>
                    <a href="{{ route('admin.settings.factory') }}"
                        class="{{ $navClass(request()->routeIs('admin.settings.*')) }}"><i
                            data-lucide="settings"></i><span class="sidebar-label">Settings</span></a>
    ...
    <div class="border-t border-white/10 p-3">
        <div class="flex items-center gap-3 rounded-2xl bg-white/[.06] p-2.5">
            <span
                class="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-white/10 text-sm font-extrabold">{{ strtoupper(substr(Auth::user()->name, 0, 1)) }}</span>
            <span class="sidebar-label min-w-0 flex-1"><span
                    class="block truncate text-xs font-bold">{{ Auth::user()->name }}</span><span
                    class="block truncate text-[10px] capitalize text-slate-500">{{ Auth::user()->roles->first()?->name }}</span></span>
