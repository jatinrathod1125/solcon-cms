...
class UpdateEpoxyComponentRequest extends FormRequest
{
    /**
    ...
    {
        $componentId = $this->route('epoxy_component')?->id ?? $this->route('epoxyComponent')?->id;

        return [
            'brand_id' => ['nullable', Rule::exists('brands', 'id')->where('is_active', true)],
            'name' => ['required', 'string', 'max:255'],
            'code' => ['required', 'string', 'max:50', Rule::unique('epoxy_components', 'code')->ignore($componentId)],
            ...
            'default_packing' => ['nullable', 'string', 'max:50'],
